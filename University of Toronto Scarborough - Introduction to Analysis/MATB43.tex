...
$exp(x)=\sum^\infty_{n=0} \frac{x^n}{n!}$ converges on $\mathbb{R}$\\
$e = exp(1) = \sum^\infty_{n=0} \frac{1}{n!}$\\
checked: $e^{\frac{l}{m}} = exp(\frac{l}{m}), l,m\in\mathbb{Z}, m\neq 0$\\
We can then define $e^r, r\in\mathbb{R}, e^r = exp(r)$
...
The 3rd and 4th properties imply that $e$ is bijective. So for any $y>0$, there exists a unique $x : e^x = y \Longrightarrow e$ has an inverse.

\subsection{Logarithm}

$$\log x = \int^x_1 \frac{dt}{t}, x > 0$$

\begin{proposition}
	$\log (e^x) = x, \forall x\in\mathbb{R}$
\end{proposition}

\begin{proof}
	\begin{align*}
		\frac{d}{dx} \log (e^x) &= \log' (e^x) \frac{d}{dx} e^x\\
		&= \frac{1}{e^x} e^x = 1
	\end{align*}
\end{proof}

Now set $x=0$, $\log (e^0) = \log (1) = 0 \Longrightarrow c=0 \Longrightarrow \log (e^x) = x$. So $\log$ is an invserse to $e^x$ and vice versa.

\subsubsection{Properties of the Logarithmic Function}
...
\begin{proposition}
	$$e = \lim_{t\to 0} (1+t)^{\frac{1}{t}} = \lim_{n\to\infty} (1 + \frac{1}{n})^{\frac{1}{n}}$$
\end{proposition}

\begin{proof}
	\begin{align*}
		\log(\lim_{t\to 0} (1+t)^{\frac{1}{t}}) &\overset{\text{continuous}}{=} \lim_{t\to 0}\log((1+t)^{\frac{1}{t}})\\
		&= \lim_{t\to 0} \frac{1}{t} \log(1+t)\\
		&= \lim_{t\to 0} \frac{\log(1+t) - \log(1)}{t}\\
		&= \lim_{t\to 0} \frac{\log(1+t)}{t}\\
		&= \log (\lim_{t\to 0} (1+t)^{\frac{1}{t}})\\
		&\Longrightarrow e = (\lim_{t\to 0} (1+t)^{\frac{1}{t}})
	\end{align*}
	Since $[\frac{d}{dt} (\log t)]_{t=1} = 1 = \log (e) = 1$ and $\log$ is a bijection
\end{proof}

\subsection{Trigonometric Numbers}

\begin{definition}
	$$\sin x = 1 - \frac{x^3}{3!} + \frac{x^5}{5!} - ... = \sum^\infty_{n=0} (-1)^n \frac{x^{2n+1}}{(2n+1)!}$$
\end{definition}

Note that the ratio test implies that a series converges for all $x\in\mathbb{R}$

\begin{definition}
	$$\cos x = 1 - \frac{x^2}{2!} + \frac{x^4}{4!} - ... = \sum^\infty_{n=0} (-1)^n \frac{x^{2n}}{(2n)!}$$
\end{definition}

\begin{definition}
	(Derivatives of $\sin, \cos$)
	$$\frac{d}{dt} \sin x = \cos x, \frac{d}{dt} \cos x = -\sin x$$
\end{definition}

Note that:

$$\cos (ix) = \sum^\infty_{n=0} (-1)^n \frac{(ix)^{2n}}{(2n)!} = \sum^\infty_{n=0} \frac{x^{2n}}{(2n)!}$$
$$\sin (ix) = \sum^\infty_{n=0} (-1)^n \frac{(ix^{2n+1})}{(2n+1)!}$$

Then
$$e^{ix} = \sum^\infty_{i=0} \frac{(ix)^n}{n!} = \cos x + i \sin x$$
...
\begin{align*}
	...
	&\Longrightarrow e^{i (x+y)} = (\cos x\cos y - \sin y \sin x)+i(\sin x \cos y + \cos x \sin y)
\end{align*}
...
$$\Longrightarrow \cos (x) = \frac{e^{ix} + e^{-ix}}{2}, \sin (x) = \frac{e^{ix} - e^{-ix}}{2}$$
...
\subsection{Open and closed subsets of the real numbers}

\begin{definition}
  $U \subset \mathbb{R}$ is an open set if, for any $x\in U$, there exists $\epsilon > 0$ such that $(x-\epsilon, x+\epsilon) \subset U$
\end{definition}
...
\begin{example}
  $\mathbb{R} \setminus \{ 0 \}$ is an open set
\end{example}
...
\begin{proposition}
  Suppose $U_a, a\in A$ is a collection of open sets $\Longrightarrow$
  $$U = \bigcup_{a\in A} U_a \text{ is open}$$
\end{proposition}

\begin{proof}
  Suppose $x\in U$, then $x \in U_a$ for some $a \in A$. Since $U_a$ is open, there exists a neighbourhood $(x-\epsilon, x+\epsilon) \subset U_a$ for some $\epsilon > 0$. Therefore $(x-\epsilon, x+\epsilon) \subset U = \bigcup_{a\in A} U_a$
\end{proof}

\begin{proposition}
  Suppose $U_1,..., U_n$ are open sets then $$U = \bigcap_{i=1}^m U_i \text{ is open}$$
\end{proposition}

\begin{proof}
  Suppose $x \in U = \bigcap_{i=1}^m U_i$ $\Longrightarrow x\in U_i$ for each i, $U_i$ is open $\Longrightarrow$ there exists a neighbourhood $(x-\epsilon_i, x+\epsilon_i), \epsilon_i > 0$\\
  \\
  Take $\epsilon = sup(\epsilon_i), \forall i, (i \in [1,m])$ so $\epsilon > 0$.\\
  (Since $\forall \epsilon$ sits inside all sets, openness is proven).\\
  for all $i$,
  $$(x-\epsilon, x+\epsilon) \subset (x-\epsilon_i x+\epsilon_i) \subset U_i$$
  $$\Longrightarrow (x-\epsilon, x+\epsilon) \subset \bigcap_{i=1}^m U_i$$
\end{proof}

\begin{example}
  let $U_i = (-\frac{1}{i}, \frac{1}{i}), i\in \mathbb{N}$
  $$\Longrightarrow \bigcap_{i\in\mathbb{N}} U_i = \{ 0 \} \text{ which contains no neighbourhood, therefore not open}$$
  So infinite examples may not work
\end{example}

\begin{proposition}
  Suppose $U\subset\mathbb{R}$ is open, then $U$ is the union of a finite or countablecollection of pairwise disjoint open intervals (take two open intervals, and they cannot intersect).
\end{proposition}

\begin{proof}
  Define a relation on $U$, $a,b \in U, a\sim b$ if all points between $a,b$
 lie in $U$. We must prove $\sim$ is reflexive, symmetric, and transitive to show that it is also an equivalence relation. Assume $a<b<c$, if all the points in $a$ & $b$ lie in $U$, if all the points in $b$ & $c$ lie in $U$, and if all the points in $a$ & $c$ lie in $U$, then $\sim$ is an equivalence relation in $U$.\\
 Equivalence classes are clearly an open interval and they are pairwise disjoint. Each open interval contains a relational number, list these numbers, there are countably many or finitely many in the list.
\end{proof}

\begin{definition}
  A set $S\subset \mathbb{R}$ is \textbf{Closed} if $U = \mathbb{R}\setminus S$ is open
\end{definition}

\begin{example}
  $[a,b]$ is closed because it's compliment is open
\end{example}

\begin{example}
  $[b,\infty]$ is closed because it's compliment is open
\end{example}
...
\begin{proposition}
  $S_a, a\in A$ is a collection of closed sets. $$\Longrightarrow \bigcap_{a\in A} S_a \text{ is closed}$$
\end{proposition}

\begin{proof}
  let $U_a = \mathbb{R} \setminus S_a$, and since $\mathbb{R} \setminus \bigcap_{a\in A} S_a$ is open, then $S$ must be closed
\end{proof}

\begin{proposition}
  $S_1,...,S_m$ is closed $\Longrightarrow S = \bigcup_{i=1}^m S_i$ is also closed
\end{proposition}

\begin{proof}
  let $U_i = \mathbb{R} \setminus S_i, i \in \mathbb{N}$.
  $$\mathbb{R} \setminus \bigcap^m_{i=1} U_i = \bigcup^m_{i=1} S_i$$
  $$\bigcap^m_{i=1} U_i \text{ is open} \Longrightarrow \bigcup^m_{i=1} S_i \text{ is closed}$$
\end{proof}
...
\section{April 2, 2018}

\subsection{Uniform Continuity}

\begin{definition}
	(Uniform continuity) $S\subset\mathbb{R}, f:S \mapsto\mathbb{R}, x_0 \in S$\\
	$f$ is continuous at $x_0$, given $\epsilon > 0$, there exists $\delta > 0$ such that, for $|x-x_0|<\delta, |f(x)-f(x_0)|<\epsilon$, $\delta$ depends on $x_0$
\end{definition}

\begin{example}
	$f(x) = \frac{1}{x}, x\in (-1,0)$, regard $\epsilon$ as fixed as $x_0$ gets closer to 0, the slope of the graph $\frac{1}{x_0^2}$ increases so $\delta$ must get smaller:\\
	Using Mean Value Theorem:

	$|f(x) - f(x_0)| = f'(x_1)(x-x_0)$ where $x_1$ lies inbetween $x, x_0$.\\
	$\epsilon = f'(x_1)\delta$\\
	The choice of $\delta$: as $x_0$ gets closer to 0, $f'(x_0)= \frac{1}{x_0^2}$ gets larger, so $\delta$ must get smaller.\\
	\\
	However on $(-2, -1)$. $f'(x_0) = \frac{1}{x_0^2} \leq 1 \Longrightarrow |f(x)-f(x_0)| \leq |x - x_0|$. So if we want $|f(x)-f(x_0)|<\epsilon$, we need only take $|x-x_0|< \epsilon,$ i.e. $\delta = \epsilon$
\end{example}

\begin{definition}
	$f$ is \textbf{uniformly continuous} on $S$, if given $\epsilon > 0$, there exists $\delta > 0$ such that $|f(y)-f(x)|< \epsilon$ if $|y-x|<\delta$\\
	i.e. $\delta$ does not depend on $x$
\end{definition}

\begin{example}
	on $(-2, -1), f(x)=\frac{-1}{x}$ is uniformly continuous, but on $(-1,0)$ it is not.
\end{example}

\begin{example}
	$S = \mathbb{R}, f(x)=\sin (x)$\\
	then $|\sin x - \sin y| = |\cos z| |x-y| \leq |x-y|$\\
	$z$ is between $x,y$ so to achieve $|\sin x - \sin y|<\epsilon$, take $|x-y|<\epsilon$
\end{example}
...
\begin{theorem}
	$f$ uniformly continuous on $S \Longrightarrow$ $f$ continuous on $S$.
\end{theorem}

\begin{remark}
	$f(x) = \frac{-1}{x}$ being uniformly continuous on $[-2,-1]$ is not a coincidence
\end{remark}

\begin{theorem}
	$K \subset \mathbb{R}$ is compact, $f:K \mapsto \mathbb{R}$ is continuous $\Longrightarrow f$ is uniformly continuous.
\end{theorem}

\begin{proof}
	 need to show: given $\epsilon > 0$, there exists $\delta > 0$ so that
	 $$|x-y| < \delta \Longrightarrow |f(x) - f(y)| < \epsilon, \forall x,y\in K$$
	 suppose this condition does not hold for a given $\epsilon > 0$\\
	 \\
	 i.e. For any $\delta > 0, \exists x,y \in K$ with $|f(x)-f(y)| \geq \epsilon$. In particular, for $\delta = \frac{1}{n}, \exists$ points $x_n, y_n \in K$ with $|x_n - y_n| < \frac{1}{n}$ but $f(x_n)-f(y_n) \geq \epsilon$ so $\{ x_n \} \subset K$ is compact.\\
	 $\Longrightarrow$ there exists a subsequence $\{ x_{n_k} \} \to a \in k$\\
	 $\Longrightarrow  \{ y_{n_k} \} \to a$ as well.\\
	 \\
	 $f$ is continuous implies that $f(x_{n_k}) \to f(a)$ and $f(y_{n_k}) \to f(a)$ as $k\to\infty$. So for a large enough $k$
	 $|f(x_{n_k})-f(y_{n_k})|<\epsilon$\\
	 $\Longrightarrow$ $f$ is in fact uniformly continuous on $K$.
\end{proof}

\subsection{The limit of the Supremum and Infimum}

given a sequence $\{ S_n \} \subset \mathbb{R}$, bounded below.
$$A_N = \inf \{ S_{N+1}, S_{N+2},...\} = \inf_{n > N} \{ S_n \}$$

Note that $A_N \leq A_{N+1} \leq  ...$ is an increasing sequence

Define $$\lim \sup \{ S_n \} = \lim_{n\to\infty} A_n$$
if $S_n$ is not bounded above then $\lim \sup \{ S_n \} = \infty$

\newpage

\section{Friday, April 6, 2018}

\subsection{the Limit of the Supremum and the Limit of the Infimum}

\begin{note}
	If $S_n$ is not bounded above, then the limit of the Supremum of $S_n$ is $\infty$\\
	If $S_n$ is not bounded below, then the limit of the Infimum of $S_n$ is $-\infty$
\end{note}

Consider the bounded sequence $\{ S_n \}$.\\
Let $A_N = \sup \{ S_n | n > N \}$
$A_N$ forms a decreasing Sequence and converges, define
$$\lim_{n\to\infty} \sup S_n = \lim_{N\to\infty} A_N$$

Similarly, $B_N = \inf \{ S_n | n > N \}$, $B_N$ is an increasing sequence bounded above, Let
$$\lim_{n\to\infty} \inf S_n = \lim_{N\to\infty} B_N$$

\begin{example}
	$$S_n = (-1)^n, \lim_{n\to\infty} \sup S_n = 1, \lim_{n\to\infty} \inf S_n = -1$$
\end{example}

\begin{example}
	$$S_n = n (1+(-1)^n) = \begin{cases}
		2n, &\text{if $n$ even}\\
		0, &\text{if $n$ odd}
	\end{cases}, \lim_{n\to\infty} \sup S_n = \infty, \lim_{n\to\infty} \inf S_n = 0$$
\end{example}

\begin{example}
	Let $S_n = \sin(\frac{n\pi}{3})$. The only possible solutions are $\pm \frac{\sqrt{3}}{2}, 0$, so
	$$\lim_{n\to\infty} \sup S_n = \frac{\sqrt{3}}{2}, \lim_{n\to\infty} \inf S_n = -\frac{\sqrt{3}}{2}$$
\end{example}

\begin{proposition}
	For any sequence $S_n$, there exists a subsequence, whose limit is $lim_{n\to\infty} \sup S_n$ and one whose limit is $lim_{n\to\infty} \inf S_n$
\end{proposition}

\begin{proof}
	Assume $\lim_{n\to\infty} \sup S_n \neq \infty$. Let $a= \lim_{n\to\infty} \sup S_n$, so $a = \lim_{N\to\infty} A_N$\\
	So given $K$, there exists $M$, such that for $N \geq M, |a_k - a| < \frac{1}{k}$\\
	\\
	Now, $A_m = \sup_{n > M} \{ s_n \} \Longrightarrow$ There exists $m_k >  M_k$ such that $|A_m - S_{m_k} |< \frac{1}{2k}$
	$$|S_{n_k} - a| \leq  | S_{n_k} - A_m | + |A_m - a| < \frac{1}{k}, S_{n_k} \to a \text{ as } k\to\infty$$

	Analogous construction for $\lim\inf S_n$
\end{proof}

\begin{remark}
	For any $N$, $B_N \leq A_N$\\
	$$\Longrightarrow \lim_{n\to\infty} \inf S_n = \lim_{n\to\infty} B_n \leq \lim_{n\to\infty} A_n = \lim_{n\to\infty} \sup S_n$$
\end{remark}

\begin{proposition}
	Suppose $\{ S_n \}$ converges to a, then $$\lim_{n\to\infty} \sup S_n = a = \lim_{n\to\infty} \inf S_n$$
\end{proposition}

\begin{proof}
	Since $\{ S_n \} \to a$, every subsequence to $a$ in particular, a substance which converges to $\lim_{n\to\infty} \sup S_n$, converges to $a, \Longrightarrow$ $a=\lim_{n\to\infty} \sup S_n$
\end{proof}

\textbf{There's something here I'm missing}

\begin{proof}
 let $\{ S_{n_k} \}$ be a convergent subsequence, then
 $$\lim_{k\to\infty} S_{n_k} = \lim_{k\to\infty} \sup(S_{n_k}) \leq \lim_{n\to\infty} \sup (S_n) \Longrightarrow \sup (S) \leq \lim_{n\to\infty} \sup (S)$$
 But there exists a convergent subsequence whose limit is
 $$\lim_{n\to\infty} \sup (S) \Longrightarrow \sup(S) = \lim_{n\to\infty} \sup (S_n)$$

 Similar argument for $\inf$ case.
\end{proof}

\subsection{Root Test Revisited}

Given a series $\sum^\infty_{n=1} a_n$

\begin{proposition}
	If $$A = \lim_{n\to\infty} \sup |a_n|^{\frac{1}{n}} < 1$$
	then $\sum_n a_n$ converges. If $A > 1$ then$\sum_n a_n$ diverges.
\end{proposition}

\begin{proof}
	Let $A_n = \inf_{n \geq N} \{ |a_n|^{\frac{1}{n}} \}$ so $$\lim_{n\to\infty} \inf |a_n|^{\frac{1}{n}} = \lim_{N\to\infty} A_N$$Suppose
	$$\lim_{n\to\infty} \inf |a_n|^{\frac{1}{n}} < 1$$\\
	Pick a $c$ such that
	$$\lim_{n\to\infty} \inf |a_n|^{\frac{1}{n}} < c < 1$$
	then there exists an $M$ such that for $N \geq M, A_n < c, since $A_M = \sup_{n \geq M} \{ |a_n|^{\frac{1}{n}}$
	$$\Longrightarrow |a_n|^{\frac{1}{n}} <  c \text{ for } n \geq M$$
	$$\Longrightarrow |a_n| <  c^n \text{ for } n \geq M$$
	since $\sum_{n \geq M} c^n$ converges, so does $\sum_{n \geq M} a_n \Longrightarrow \sum^\infty_{n=0} a_n$ converges.\\
	\\
	Just do $$\lim_{n\to\infty} \sup |a_n|^{\frac{1}{n}} > c > 1$$
\end{proof}

\subsection{Radius of convergent power series}

$$\sum^\infty_{n=0} a_n x^n, c = \lim_{n\to\infty} \sup |a_n|^{\frac{1}{n}}$$
$R = \frac{1}{c}$ if $c\neq 0$ is the radius of convergence, if $c=0,$ then $R = (-\infty, \infty)$
