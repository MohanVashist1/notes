...
\\
\\
Now the Client checks if they both have the same master-key, and sends the server a message encrypted with the master-key (with a function called \codeword{ChangeCipher}). The Master then recieves this message and decrypts it. If it is successful in decrypting the message, then the server sends it's own encrypted message with the master-key to the client (with a function, again called \codeword{ChangeCipher}).
\\
\\
