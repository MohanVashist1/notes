...
\paragraph{Repalcement Policy} what page to evict to make room?
...
This method is predicting what future pages will be used at time of current fault, however, we may not know on which variables to base our predictions on and what to do if the predictions are wrong.
...
We want to replace the page that is "old enough". We do this by arranging all of the physical page frames in a big circle (like a clock) and we sweep through the pages in a circular order like a clock, and if ther reference bit is off, that means it has not been used recently and we exict that page. If there reference bit is on, that means it has been used recently andd we turn it off to go to the next page. So the clock hand moves quickly when pages are needed, and there is low overhead when there is plenty of memory.
...
There are w few problems with Working sets, such as
\begin{itemize}
...
\\
Page replacement algorithms avoid thrashing. Thrashing casses the system to be overcommitted, since the Os has no idea which pages should be in memory to reduce faults, and it could just be that there isn't enough physical memory for all of the processes in the system.\\
\\
...
\subsubsection{Linux Paging}

Linux uses a global replacement, like most Unix systems. It uses a modified second-change clock algorithm, in which pages "age" with each pass of the clock hand and pages that are not used for a long time will eventually have a value of zero.
